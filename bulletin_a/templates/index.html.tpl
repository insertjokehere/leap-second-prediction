      ...
      <div class="container py-4">
        ...
        <div class="p-5 mb-4 bg-light rounded-3">
          <div class="container-fluid py-5">
            <p class="col-md-8 fs-6">Based on <a href="{{ latest.source.url }}">IERS Bulletin A Vol. {{ latest.source.volume }} No. {{ latest.source.number }}</a>, and making some very large, probably unjustified assumptions;</p>
            {% if latest.next_leap_second %}
            {% with next=latest.next_leap_second.predicted_leap_day %}
            <p class="col-md-8 fs-4">at <b>the end of {% if next.month == 7 %}June, {{ next.year }}{% else %}December, {{ next.year - 1 }}{% endif %}</b></p>
            {% endwith %}
